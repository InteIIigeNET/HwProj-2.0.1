...
namespace HwProj.CourseWorkService.API.Services.Interfaces
{
	public interface ICourseWorksService
	{
		...
		Task<long> AddCourseWorkAsync(CreateCourseWorkViewModel courseWorkViewModel, string userId,
			bool createdByCurator);
		...
		Task UpdateReferenceInCourseWorkAsync(string userId, long courseWorkId, string reference = null,
			bool remove = false);
