...
namespace HwProj.CoursesService.API.Controllers
{
    ...
    public class RegistrationRequestsController : Controller
    {
        ...
        public async Task<IActionResult> Create([FromBody] CreateRegistrationRequestViewModel model)
        {
            var result = await _service.CreateRequestAsync(model);
            if (!result.Succeeded)
            {
        ...
        // [ProducesResponseType(typeof(Result<RegistrationRequestDto?>), (int)HttpStatusCode.OK)]
        // public async Task<IActionResult> GetRequestByEmail(string email)
        // {
        //     var result = await _service.GetRequestByEmailAsync(email);
        //     return Ok(result);
        // }
