...
namespace HwProj.CoursesService.API.Repositories
{
    public class DeadlinesRepository : CrudRepository<Deadline, long>, IDeadlinesRepository
    {
        ...
        public async Task<List<Deadline>> GetTaskDeadlinesForStudent(string studentId, long taskId)
        {
            return await Context.Set<Deadline>()
                .Where(d => d.AffectedStudentsId.Contains(studentId) && d.TaskId == taskId)
                .ToListAsync();
