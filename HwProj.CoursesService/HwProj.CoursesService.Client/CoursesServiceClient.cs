...
namespace HwProj.CoursesService.Client
{
    public class CoursesServiceClient : ICoursesServiceClient
    {
        ...
        public async Task<long> AddHomeworkToCourse(CreateHomeworkViewModel model, long courseId)
        {
            using var httpRequest = new HttpRequestMessage(
                HttpMethod.Post,
                _coursesServiceUri + $"api/Courses/{courseId}/Homeworks/add")
            {
                Content = new StringContent(
                    JsonConvert.SerializeObject(model),
                    Encoding.UTF8,
        ...
        public async Task UpdateHomework(CreateHomeworkViewModel model, long courseId, long homeworkId)
        {
            using var httpRequest = new HttpRequestMessage(
                HttpMethod.Put,
        ...
        public async Task DeleteHomework(long courseId, long homeworkId)
        {
            using var httpRequest = new HttpRequestMessage(
                HttpMethod.Delete,
                _coursesServiceUri + $"api/Courses/{courseId}/Homeworks/delete/{homeworkId}");
            
            await _httpClient.SendAsync(httpRequest);
        }

        public async Task<long> AddTask(long homeworkId, CreateTaskViewModel taskViewModel)
        {
            using var httpRequest = new HttpRequestMessage(
                HttpMethod.Post,
        ...
        public async Task UpdateTask(long homeworkId,
