        ...
        public async Task HandleAsync(StudentPassTaskEvent @event)
        {
            foreach (var m in @event.Course.MentorIds.Split('/'))
                await _notificationRepository.AddAsync(new Notification
                {
                    ...
                    Body =
                        $"{@event.Student.Name} {@event.Student.Surname} добавил(-а) новое <a href='{@event.Solution.GithubUrl}' target='_blank'>решение</a> задачи <a href='task/{@event.Task.Id}'>{@event.Task.Title}</a> из курса <a href='courses/{@event.Course.Id}'>{@event.Course.Name}</a>.",
