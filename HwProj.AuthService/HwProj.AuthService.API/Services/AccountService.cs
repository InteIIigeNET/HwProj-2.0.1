...
public class AccountService : IAccountService
{
    ...
    public async Task<Result> ResetPassword(ResetPasswordViewModel model)
    {
        ...
        if (model.Password.Length < 6) return Result.Failed("Пароль должен содержать не менее 6 символов");

        if (model.Password != model.PasswordConfirm) return Result.Failed("Пароль и его подтверждение не совпадают");
