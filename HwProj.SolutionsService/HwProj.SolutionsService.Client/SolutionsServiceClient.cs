...
namespace HwProj.SolutionsService.Client
{
    public class SolutionsServiceClient : ISolutionsServiceClient
    {
        ...
        public async Task<StatisticsCourseMatesDto[]> GetCourseStatistics(long courseId, long taskId, string userId)
        {
            using var httpRequest = new HttpRequestMessage(
                HttpMethod.Get,
                _solutionServiceUri + $"api/Solutions/getCourseStat/{courseId}/{taskId}?userId={userId}");
